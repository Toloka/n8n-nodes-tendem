...
settings: org `Toloka`, repository `n8n-nodes-tendem`, workflow
`publish-n8n-tendem.yml`, no environment. The workflow carries no
`NODE_AUTH_TOKEN` and needs no secret.

**Why this package lives in its own repo (moved 2026-08-19 from
`Toloka/tendem-mcp`, where it was `integrations/n8n-nodes-tendem/`):** the n8n
Creator Portal's automatic vetting failed the monorepo with "Can't find
credential file in repo" — it expects the scaffold layout (`package.json`,
`nodes/`, `credentials/` at the repo root) and does not honour
`repository.directory`. Release 0.1.3 is the first from this repo; the npm
trusted publisher must point here.
